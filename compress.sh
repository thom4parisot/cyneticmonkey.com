...
rm ${CSS}/screen.css
cat index-dev.html | sed -r '/<link rel="stylesheet"[^>]+ \/>/d' | sed -r "s/\t<title>/\t<link rel=\"stylesheet\" media=\"all\" type=\"text\/css\" href=\"\/assets\/stylesheets\/screen.min.css?v=${VERSION}\" \/>\r\n\t<title>/" > index.html